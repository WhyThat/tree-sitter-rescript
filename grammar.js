module.exports = grammar({
  ...
  rules: {
    ...
    module_binding: ($) =>
      prec.left(
        seq(
          ...
          optional(seq("=", field("definition", $._module_definition))),
        ),
      ),
    ...
    _module_structure: ($) =>
      seq($._module_definition, optional($.module_type_annotation)),

    _module_definition: ($) =>
      choice($.block, $.module_expression, $.functor, $.extension_expression),
    ...
    _type: ($) =>
      choice($._inline_type, $.variant_type, $.record_type, $.as_aliasing_type),
    ...
    record_type: ($) => seq("{", commaSept($.record_type_field), "}"),
    ...
    object_type_field: ($) =>
      choice(
        ...
        seq(alias($.string, $.property_identifier), ":", $._type),
      ),

    generic_type: ($) => prec.left(seq($._type_identifier, $.type_arguments)),

    type_arguments: ($) => seq("<", commaSep1t($._type), ">"),

    function_type: ($) =>
      prec.left(seq($.function_type_parameters, "=>", $._type)),

    function_type_parameters: ($) =>
      choice($._non_function_inline_type, $._function_type_parameter_list),

    _function_type_parameter_list: ($) =>
      seq("(", commaSept(alias($.function_type_parameter, $.parameter)), ")"),
    ...
    value_identifier_path: ($) =>
      seq($.module_primary_expression, ".", $.value_identifier),
    ...
    record_field: ($) =>
      seq($._record_field_name, ":", optional("?"), $.expression),

    _record_pun_field: ($) => seq(optional("?"), $._record_field_name),

    _record_single_field: ($) => seq($.record_field, optional(",")),

    _record_single_pun_field: ($) =>
      seq("?", $._record_field_name, optional(",")),
    ...
    object_field: ($) =>
      seq(alias($.string, $.property_identifier), ":", $.expression),
    ...
    _labeled_parameter_default_value: ($) =>
    ...
    _variant_pattern_parameters: ($) =>
      seq("(", commaSept($._variant_pattern_parameter), ")"),
    ...
    tuple_pattern: ($) => seq("(", commaSep2t($.tuple_item_pattern), ")"),

    array_pattern: ($) =>
      seq("[", optional(commaSep1t($._collection_element_pattern)), "]"),
    ...
    _jsx_identifier: ($) =>
      alias(choice($.value_identifier, $.module_identifier), $.jsx_identifier),
    ...
    mutation_expression: ($) =>
      seq($._mutation_lvalue, choice("=", ":="), $.expression),

    _mutation_lvalue: ($) =>
      choice($.value_identifier, $.member_expression, $.subscript_expression),
    ...
    member_expression: ($) =>
      prec(
        ...
        seq(
          ...
          ".",
          optional(seq(field("module", $.module_identifier), ".")),
    ...
    nested_variant_identifier: ($) =>
      seq($.module_primary_expression, ".", $.variant_identifier),

    variant_arguments: ($) =>
      seq("(", commaSept(seq($.expression, optional($.type_annotation))), ")"),
    ...
    module_identifier_path: ($) =>
      path($.module_primary_expression, $.module_identifier),
